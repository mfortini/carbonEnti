...
```sql
PIVOT entiRes  ON starts_with(url, 'https://') USING COUNT(Codice_IPA) GROUP BY crawlDate ORDER BY crawlDate
```
...
```sql
PIVOT (SELECT * FROM entiRes WHERE bootstrap2_js IS NOT NULL OR bootstrap2_css IS NOT NULL) ON (bootstrap2_js IS NOT NULL OR bootstrap2_css IS NOT NULL) USING COUNT(Codice_IPA) GROUP BY crawlDate
```
